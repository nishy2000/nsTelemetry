@ECHO OFF
REM Copyright 2020, 2021, 2022, 2023 nishy software
...
ECHO -----------------------
IF "x-%NO_BUILD%" == "x-" (
	dotnet.exe build -c %TARGET_CONFIG% --no-incremental --no-dependencies nsTelemetry.csproj
	IF ERRORLEVEL 1 GOTO ERROR
) ELSE (
...
REM  NishySoftware.Telemetry modules
...
REM  NishySoftware.Telemetry modules
COPY "%BIN_FOLDER%\%RUNTIME_ID_FRM%\nsTelemetry.dll" "%SIGNED_FOLER%\%RUNTIME_ID_FRM%\" >nul
IF ERRORLEVEL 1 GOTO ERROR
COPY "%BIN_FOLDER%\%RUNTIME_ID_NET%\nsTelemetry.dll" "%SIGNED_FOLER%\%RUNTIME_ID_NET%\" >nul
IF ERRORLEVEL 1 GOTO ERROR
...
dotnet.exe pack -c %TARGET_CONFIG% --no-build -o "%WORK_FOLDER%" nsTelemetry.csproj
IF ERRORLEVEL 1 GOTO ERROR
